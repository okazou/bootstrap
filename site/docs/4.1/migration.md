...
title: バージョン4への移行
description: Bootstrap 4 は全体的に全て書き換えています。主な変更を以下に要約し, 関連するコンポーネントの具体的な変更内容を記します。
group: migration
toc: true
---
...
Beta3 から安定版は大きな変更はありませんが、注目すべきいくつかの変更があります。
...
- 印刷ユーテリティの問題を修正しました。以前は `.d-print-*` を使うと他の `.d-*` よりも優先されていました. 今は印刷(`@media print`)のときだけ適用されます。
- 印刷ユーテリティを拡張しました。Beta3 以前は `block`, `inline-block`, `inline`, そして `none` だけでした。安定版では `flex`, `inline-flex`, `table`, `table-row`, そして `table-cell` を追加しました。
- `@page` `size` で指定した印刷プレビューのレンダリングを修正しました。
...
Beta2 では大きな変更がありましたが, Beta3 では更に対応しないければいけないものがいつくかあります。
...
- 未使用の余分な `$thumbnail-transition` 変数を削除しました。
- npm パッケージは, ソースと dist ファイルだけです。それ以外のファイルに依存して `node_module` で実行している場合は, 開発環境を調整する必要があります。
...
- 標準およびカスタムフォームのチェックボックスとラジオボタンを書き直しました。同じ HTML 構造とスタイルが一致しています。そのため, 入力状態によってラベルを変更することができ, `disabled` 属性が使いやすくなり, バリデーションもサポートしやすくなりました。
  ...
  カスタムチェックインラインを作成するには `.custom-control-inline` を追加します。

- input ベースのボタングループのセレクタを更新しました。JavaScript/CSS は `[data-toggle="buttons"] { }` を使いません。JavaScript は `data` 属性を使い, CSS は `.btn-group-toggle` を使います。

- `.col-form-legend` を削除し, `.col-form-label` をちょっと改善しました。`.col-form-label-sm` と `.col-form-label-lg` はn `<legend>` 要素で簡単に使用できます。

- カスタムのファイルインプットは Sass 変数の `$custom-file-text` で変更できるようになります。ブラウザの `Choose file` テキストは `.custom-file-label` から取得されるようになります。
...
- インプットグループのアドオンはインプットフィールドに対する配置固有のものになりました。`.input-group-addon` と `.input-group-btn` を削除し, `.input-group-prepend` と `.input-group-append` を追加しましうた。明示的に append および prepend を使えば CSS を単純化できます。prepend/append の中では `.input-group-text` でテキストをラップします。

- 複数のフォーム要素を対象にバリデーションスタイルがサポートされるようになりました。(ただし, グループごとに1つのフォーム要素のみ)

- サイズはそれぞれのフォーム要素ではなく, 親の `.input-group` に指定する必要があります。
...
Beta 版ではなるべく大きな変更をしないように留意しています。しかし, 物事は計画通りに進むとは限りません。以下は Beta から Beta2 に移行する際の大きな変更点です。
...
- `.badge` における `$badge-color` と使い方を削除しました。`background-color` に基づく `color` を使うので変数は不要です。
- CSS の `grayscale` フィルタとの衝突を回避するために `grayscale()` 関数を `gray()` にリネームしました。
- `.table-inverse`, `.thead-inverse`, そして `.thead-default` を `.*-dark` と `.*-light` に変更しました。
- レスポンシブテーブルは各ブレイクポイントごとに生成されるようになりました。Beta では `.table-responsive` だったものが `.table-responsive-md` のようになります。必要に応じて `.table-responsive` または `.table-responsive-{sm,md,lg,xl}` を使うことができます。
- パッケージマネージャ Bower のサポートをやめました。(代替のパッケージマネージャ: Yarn, npm) [詳細は bower/bower#2298](https://github.com/bower/bower/issues/2298)
- Bootstrap には jQuery 1.9.1 以降が必要ですが, バージョン3 以降を推奨します。バージョン3 以降は Bootstrap と同じサポートブラウザで, いくつかのセキュリティ上の対策がされているからです。
- 未使用の `.form-control-label` を削除しました。これは　`<label>` と垂直方向に配置した `.col-form-label` と重複しているからです。
- `color-yiq` を `color` を含む mixin から関数に変更し、あした。これを使えば任意の CSS プロパティに使用できます。例えば `color-yiq(#000)` の代わりに `color: color-yiq(#000);` のように記述できます。
...
- `.modal-dialog` で新しい `pointer-events` を導入しました。`pointer-events: none` を設定すれば, 外側の `.modal-dialog` クリックを無効にできます。`pointer-events: auto` を設定すれば有効にできます。
...
ここからは Bootstrap3 から Bootstrap4 に移行する際に, 気をつけるべき大きな変更点について紹介します。
...
- IE8, IE9, iOS6 をサポートから外しました。Bootstrap4 は IE10+ と iOS 7+ のみです。どちらかを必要とする場合は Bootstrap3 を使ってください。
- Android 5.0 Lolipop のブラウザと WebView をサポートに追加しました。以前の Android のブラウザと WebView はサポートしません。
...
- **flexbox が標準で有効になりました。** 
- CSS ソースが [Less](http://lesscss.org/) から [Sass](https://sass-lang.com/) に変更になりました。
- CSSの標準単位を `px` から `rem` に切り替えました。デバイスの viewport やメディアクエリは影響を受けないので引き続き `px` を使います。
- ベースフォントサイズを `14px`から `16px` に変更しました。
- 5 番目のオプション(`576px` 以下のデバイスに対応)を追加し, クラスから `-xs` を削除しました。 (例: `.col-6.col-sm-4.col-md-3`)
- 独立したオプションのテーマを設定可能な SCSS 変数 に置き換えました。(例: `$enable-gradients: true`)
- Grunt の代わりに npm scripts を使用するようにシステムを改良しました。すべての scripts については `package.json` を、ローカル開発のニーズについてはプロジェクトの readme を参照してください。
- Bootstrap のレスポンシブ無効化はサポートされなくなりました。
- オンラインカスタマイザーを廃止し, 幅広いセットアップドキュメントとカスタマイズされたビルドシステムが導入されました。
- [utility classes]({{ site.baseurl }}/docs/{{ site.docs_version }}/utilities/) に余白やパッディングの新しいユーティリティクラスが追加されました。
...
- **flexbox に移行**
  - グリッドミックスインと定義されたクラスに flexbox がサポートされるようになりました。
  - flexbox の機能として, 垂直方向および水平方向の配置がサポートされるようになりました。
- **更新されたグリッドクラス名と新しいグリッド**
  - Added a new `sm` grid tier below `768px` for more granular control. We now have `xs`, `sm`, `md`, `lg`, and `xl`. This also means every tier has been bumped up one level (so `.col-md-6` in v3 is now `.col-lg-6` in v4).
  - `xs` grid classes have been modified to not require the infix to more accurately represent that they start applying styles at `min-width: 0` and not a set pixel value. Instead of `.col-xs-6`, it's now `.col-6`. All other grid tiers require the infix (e.g., `sm`).
- **Updated grid sizes, mixins, and variables.**
  - Grid gutters now have a Sass map so you can specify specific gutter widths at each breakpoint.
