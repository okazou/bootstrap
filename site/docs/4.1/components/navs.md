...
layout: docs
title: Navs
description: ナビゲーションコンポーネントを利用するためのドキュメントと例を示します。
group: components
toc: true
---
<!-- 
...
全体にクラスが使用されるため, マークアップは柔軟に行うことができます。上記のように `<ul>` 使用するか, `<nav>` 要素を指定して独自のロールを作成します。
`.nav` のため `display: flexnav` リンクはnav項目と同じように動作しますが余分なマークアップはありません。  
...
<!-- Centered with `.justify-content-center`: -->
中心にする場合は `.justify-content-center` を使用します。
...
右にする場合は `.justify-content-end` を使用します。
...
すべての水平スペースが占有されていますが、すべてのnavアイテムが同じ幅であるわけではありません。
...
`<nav>` ベースのナビゲーションを使用する場合は `.nav-item` を含めるようにしてください。
...
等幅要素の場合は `.nav-justified` を使用します。水平スペースはnavリンクで占有されますが `.nav-fill`では上記とは異なり、すべてのnavアイテムは同じ幅になります。
...
コンパイルされた `bootstrap.js` ファイルを通じて, JavaScriptプラグインタブを使用してナビゲーションタブとピルを拡張して,ドロップダウンメニューを使用しても,タブペインのローカルコンテンツを作成できます。


<!-- If you're building our JavaScript from source, it [requires `util.js`]({{ site.baseurl }}/docs/{{ site.docs_version }}/getting-started/javascript/#util). -->

ソースをJavaScriptからビルドする場合は [requires `util.js`]({{ site.baseurl }}/docs/{{ site.docs_version }}/getting-started/javascript/#util) が必要です。

<!-- Dynamic tabbed interfaces, as described in the [<abbr title="Web Accessibility Initiative">WAI</abbr> <abbr title="Accessible Rich Internet Applications">ARIA</abbr> Authoring Practices](https://www.w3.org/TR/wai-aria-practices/#tabpanel), require `role="tablist"`, `role="tab"`, `role="tabpanel"`, and additional `aria-` attributes in order to convey their structure, functionality and current state to users of assistive technologies (such as screen readers). -->

動的なタブインタフェースは に [<abbr title="Web Accessibility Initiative">WAI</abbr> <abbr title="Accessible Rich Internet Applications">ARIA</abbr> Authoring Practices](https://www.w3.org/TR/wai-aria-practices/#tabpanel) に記載されているように `role="tablist"`, `role="tab"`, `role="tabpanel"`, `aria-`など構造や機能の現在の状態を支援技術（スクリーンリーダーなど）のユーザーに提供します。
...
動的タブ付きインタフェースにはドロップダウンメニューが含まれないようにしてください。
利便性とアクセシビリティの両方の問題が発生します。
ユーザビリティの観点からは, 現在表示されているタブのトリガ要素が（閉じたドロップダウンメニューの中にあるので）すぐには見えないという事実が混乱の原因となります。
アクセシビリティの観点からは,現在のところ,この種の構造を標準的なWAI ARIAパターンにマッピングする賢明な方法はありません。
つまり, 支援技術のユーザーには容易に理解できません。
...
`<ul>` のように上に示したように, ベースのマークアップや任意のマークアップを使って動作します。
使用している場合 `<nav>` は `role="tablist"` を直接要素に適用しないでください。
これは、要素のネイティブな役割をナビゲーションランドマークとしてオーバーライドするためです。
代わりに、代替要素（以下の例では `<div>`）に切り替え、その `<nav>` 周りを囲みます。
...
タブ要素とコンテンツコンテナをアクティブにします。
タブにはDOM内のコンテナをターゲットとする `data-target` や `href` が必要です。
...
指定されたタブを選択し関連するペインを表示します。
前に選択された他のタブは選択解除されます。
関連するペインは非表示になります。
タブペインが実際に表示される前（　`shown.bs.tab` イベントが発生する前）に呼び出し元に戻ります。
...
2. `show.bs.tab` （表示されるべきタブ）
3. `hidden.bs.tab` (前のアクティブなタブでは `hide.bs.tab` イベントと同じタブ）
4. `shown.bs.tab` (新しくアクティブな表示されたタブでは `show.bs.tab` イベントと同じもの）
